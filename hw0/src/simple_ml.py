...
def softmax_loss(Z, y):
    ...
    value_y = Z[np.arange(len(y)), y]

    return np.mean(np.log(sum_exp_Z) - value_y)
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    ### BEGIN YOUR CODE
    rows = np.shape(X)[0]
    num_iters = math.ceil(rows / batch)

    for i in range(num_iters):
        end = min(rows, (i + 1) * batch)
        num = end - (i * batch)
        batchX = X[i * batch : end, :]
        batchY = y[i * batch : end]
        expXTheta = np.exp(batchX @ theta)
        Z = expXTheta / np.sum(expXTheta, axis=1, keepdims=True)
        # Z - Iy
        Z[np.arange(len(batchY)), batchY] -= 1
        gradient = (np.transpose(batchX) @ Z) / num
        theta -= lr * gradient
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(num_iter):
        ...
        batchX = X[i * batch : end]
        batchY = y[i * batch : end]
        Z1 = np.maximum(0, batchX @ W1)
        expZ1W2 = np.exp(Z1 @ W2)
        G2 = expZ1W2 / np.sum(expZ1W2, axis=1, keepdims=True)
        G2[np.arange(len(batchY)), batchY] -= 1
        mask = np.where(Z1 > 0, 1, 0)
        G1 = mask * (G2 @ W2.T)
        gradient_G1 = (batchX.T @ G1) / num
        gradient_G2 = (Z1.T @ G2) / num
        W1 -= lr * gradient_G1
        W2 -= lr * gradient_G2
    ### END YOUR CODE
