...
__global__ void CompactKernel(const scalar_t *a, scalar_t *out, size_t size,
  ...
  /// BEGIN SOLUTION
  // thread needs to find which index
  if (gid < size) {
    size_t off_t = 0;
    size_t t = gid;
    for (int32_t i = static_cast<int32_t>(shape.size) - 1; i >= 0; --i) {
      off_t += (t % shape.data[i]) * strides.data[i];
      t = t / shape.data[i];
    }
    out[gid] = a[off_t];
  }
...
void Compact(const CudaArray &a, CudaArray *out, std::vector<int32_t> shape,
  ...
  CompactKernel<<<dim.grid, dim.block>>>(
      a.ptr + offset, out->ptr, out->size, VecToCuda(shape), VecToCuda(strides));
}
...
void EwiseSetitem(const CudaArray &a, CudaArray *out,
  ...
  EwiseSetitemKernel<<<dim.grid, dim.block>>>(
      a.ptr, out->ptr + offset, a.size, VecToCuda(shape), VecToCuda(strides));
  /// END SOLUTION
}
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray *out,
  ...
  ScalarSetitemKernel<<<dim.grid, dim.block>>>(out->ptr + offset, size, VecToCuda(shape),
                                               VecToCuda(strides), val);
  /// END SOLUTION
}
 ...
 *   - EwiseTanh
 *
 * If you implement all these naively, there will be a lot of repeated code,
 * so you are welcome (but not required), to use macros or templates to define
