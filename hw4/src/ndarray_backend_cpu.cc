...
void ProcessData(float *dst, float *src, std::vector<int32_t> shape,
                 std::vector<int32_t> strides, DataTransferType type,
                 scalar_t val) {
...
void Compact(const AlignedArray &a, AlignedArray *out,
   ...
   */
  /// BEGIN SOLUTION
  scalar_t *src = a.ptr + offset;
  scalar_t *dst = out->ptr;

  ProcessData(dst, src, shape, strides, COMPACT, 0.0f);
  /// END SOLUTION
}
...
void EwiseSetitem(const AlignedArray &a, AlignedArray *out,
   ...
   */
  /// BEGIN SOLUTION
  scalar_t *src = out->ptr + offset;
  scalar_t *dst = a.ptr;

  ProcessData(dst, src, shape, strides, EWISE_SET_ITEM, 0.0f);
  /// END SOLUTION
}
...
void ScalarSetitem(const size_t size, scalar_t val, AlignedArray *out,
  ...
  /// BEGIN SOLUTION
  ProcessData(nullptr, out->ptr + offset, shape, strides, SCALAR_SET_ITEM, val);
  /// END SOLUTION
}
...
void ReduceMax(const AlignedArray &a, AlignedArray *out, size_t reduce_size) {
  ...
  for (size_t i = 0; i < a.size; i += reduce_size) {
    scalar_t block_max = a.ptr[i];

    for (size_t j = 1; j < reduce_size; ++j) {
      block_max = std::max(block_max, a.ptr[i + j]);
    }
    out->ptr[idx++] = block_max;
  }
  /// END SOLUTION
}
...
void ReduceSum(const AlignedArray &a, AlignedArray *out, size_t reduce_size) {
  ...
  size_t idx = 0;
  for (size_t i = 0; i < a.size; i += reduce_size) {
    scalar_t block_sum = static_cast<scalar_t>(0.0f);
    for (size_t j = 0; j < reduce_size; ++j) {
      block_sum += a.ptr[i + j];
    }
    out->ptr[idx++] = block_sum;
  }
  /// END SOLUTION
}
